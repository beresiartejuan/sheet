...
export function Sidebar({
  ...
  return (
    <div className={`fixed left-0 top-0 h-full bg-gray-50 border-r border-gray-200 transition-all duration-300 z-10 ${
      isOpen ? 'w-80' : 'w-16'
    }`}>
      <div className="flex flex-col h-full">
        ...
        <div className="p-4 border-b border-gray-200">
          <div className="flex items-center justify-between">
            {isOpen && (
              <h2 className="font-semibold text-gray-900">Hojas de Trabajo</h2>
            )}
            <button
              onClick={onToggle}
                    ...
                    {isOpen && (
                      <div className="flex-1 min-w-0">
                        <div className={`font-medium truncate ${
                          activeSheetId === sheet.id ? 'text-blue-900' : 'text-gray-900'
                        }`}>
                          {sheet.name}
                        </div>
